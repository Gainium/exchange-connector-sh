...
- **WhiteBit, as two new exchanges: `whitebit` (spot) and `whitebitUsdm` (USDⓈ-M linear perps).** Full `Exchange` implementation under `src/exchange/exchanges/whitebit/`, plus a hand-rolled signing client under `src/whitebit-custom/` — no suitable npm WhiteBit package exists (unlike Kraken's `@siebly/kraken-api`), so the confirmed auth scheme is implemented directly against Node's built-in `crypto`: a `{request, nonce, ...params}` body, base64'd into `X-TXC-PAYLOAD`, signed as hex `HMAC_SHA512(payload, secret)` into `X-TXC-SIGNATURE`. **No new runtime dependency and no new env var.** There is no `whitebitCoinm` — WhiteBit has no inverse-contract product, and unlike the long-standing `krakenCoinm` member this repo does not declare an enum variant it never registers.
- **The perp side routes through a different endpoint family, not the spot endpoints with a different symbol.** WhiteBit has no separate futures account: perpetual futures and leveraged-margin-on-spot-pairs are one unified "collateral account", differentiated purely by the market name — `BTC_USDT` is leveraged margin on a spot pair, `BTC_PERP` is a perpetual, and both go through `/api/v4/order/collateral/*` and `/api/v4/collateral-account/*`. So `whitebitUsdm` means "collateral-account trading restricted to `_PERP` markets", and `openOrder` branches on the variant rather than reusing the spot create-order call. WhiteBit's plain leveraged-margin-on-spot product has no slot in Gainium's spot/usdm/coinm convention and is deliberately out of scope: this adapter never sends a non-`_PERP` symbol through a collateral endpoint. Every path the adapter touches is collected in `whitebit/endpoints.ts` so the venue surface is reviewable in one pass.
- **Candle parsing maps by explicit index, because WhiteBit's column order is not the usual one.** `GET /api/v1/public/kline` returns each row as `[time, open, close, high, low, volume_stock, volume_money]` — open/close **before** high/low, where every other adapter here reads `[time, open, high, low, close, volume]`. Copying that order swaps `close` with `high` and `low` with `close`, which does not throw and does not fail validation: it produces candles that are individually plausible and collectively wrong, and every indicator, backtest and grid built on them is wrong too with nothing anywhere to say so. Parsing is a pure function pinned by `whitebit/candle-parsing.spec.ts`.
- **Symbol mapping recovers the quote asset a perp name omits.** WhiteBit's Market Info endpoint returns spot and perpetual markets in one list, disambiguated only by the `_PERP` suffix, and a perp name carries no quote segment at all (`BTC_PERP`, not `BTC_USDT_PERP`). A naive `replace('_', '-')` yields `BTC-PERP`, a pair that exists nowhere else on the platform, so the quote is read from the market payload's own `money` field and only falls back to USDT when the payload carries nothing. Shaped like `KrakenSymbolMapper` (one cached singleton per variant) but with no asset-renaming layer — WhiteBit already uses standard tickers. Covered by `whitebit/symbol-mapping.spec.ts`.
- Rate limits are tracked per IP rather than per key, which is what WhiteBit documents: public `/api/v1/*` at 1000 req/10s, private trade-account at 12,000 req/10s, private main-account at 1,000 req/10s. Deferred calls re-ask the budget after each wait rather than sleeping once and proceeding uncharged — the same correction made for Kraken in 1.20.13, applied here from the start.
- Registration wiring (`types.ts`, `exchangeChooser.ts`, `verify.ts`, `additionalApis.ts`) is pinned by `whitebit/registration.spec.ts`. It asserts the enum's string **values**, not just its members: renaming a member is a compile error everywhere, but changing the string it maps to is not, and those two strings become a contract the moment `websocket-connector-sh` and later consumers read them.

### Notes

- **The broker/affiliate code is accepted and deliberately never attached to any request.** WhiteBit's Broker ID needs account-manager onboarding that has not happened, so `authHeaders.code` is taken for factory-signature compatibility and dropped; the affiliate family (`getUid`, `getAffiliate`, `getRebateOverview`, `getRebateRecords`, `getReferralStatus`, `setReferralCustomerId`, `getTraderSummary`) stubs "not supported" the same way Kraken's already does. This is an explicit scope decision, not an oversight.
- `futures_changeMarginType` is **not implemented**. WhiteBit documents that both cross and isolated margin modes exist but publishes no account-level endpoint to set one, and it may instead be a parameter on the collateral order endpoints; rather than guess at a trading-endpoint parameter, the method declines. `futures_changeLeverage`'s documented body carries no `market`, so leverage appears to be account-wide — its `symbols` argument is passed through for interface-shape compatibility and documented as such at the call site rather than silently dropped. `futures_leverageBracket` is a single tier derived from Market Info's `max_leverage`, since WhiteBit publishes no Binance-style notional-tiered table.
- This adapter is inert until `exchange-connector` bumps its pointer to this commit; `app-sh`, the dashboards and the backtester pick it up separately. No existing exchange's behaviour changes — everything here sits behind two new enum branches.
...
- **Kraken: the public (per-IP) rate limit is now retried with backoff instead of failing instantly (bug #181).** Kraken returns its public-endpoint limit as HTTP **200** with `{"error":["EGeneral:Too many requests"]}` in the body, so it matched neither the spot/futures strings in `retryErrors` nor the numeric `httpStatus` entries — `shouldRetry` was always false. Every rejected `/public/OHLC` call returned `NOTOK` immediately and the market-archive backfiller simply re-requested, so the egress fleet hammered Kraken continuously instead of riding the limit out: on an affected node nearly every error line carried this signature, with **zero** `Retrying after` lines, leaving candle backfill gapped and burying every other error on those nodes. The code is now in `retryErrors` and gets the same slow rate-limit pacing (3 attempts, 30s apart) as `EAPI:Rate limit exceeded`. It deliberately does **not** trigger `noteRateLimited()`: that downgrades an *account's* private REST tier, and a per-IP public rejection says nothing about any account's private budget. Covered by `src/exchange/exchanges/kraken/rate-limit.spec.ts`.

## [1.16.8] - 2026-07-29

### Added

- **Kraken spot verify now rejects keys missing the "WebSocket interface" permission (issue #167).** Such a key passes the REST balance probe, so the connection looked healthy while the user-stream connector's `GetWebSocketsToken` call was rejected with `EGeneral:Permission denied` forever — the user was never told and their bots silently fell back to delayed reconcile-sweep-only fill delivery. `verifyKraken` now calls the new `Kraken.verifyWebsocketPermission()` (a `GetWebSocketsToken` probe) after the balance check and fails with a user-facing reason naming the exact Kraken setting to enable, following the Hyperliquid agent-address guard precedent. Only a definite `EGeneral:Permission denied` rejects; transient errors (rate limit, 5xx) never block verification. Spot-only — Kraken Futures WS auth signs a challenge with the key itself and has no separate permission.
