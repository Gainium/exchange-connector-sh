...
- `GET /accountFills` falls back to the executions-history API (`api/history/v3/executions`) when Kraken rejects `/derivatives/api/v3/fills`. It does reject it: `authenticationError`, observed on two unrelated accounts whose keys serve `/accounts` perfectly well over the same egress IP with the same signing, so the fills endpoint cannot be relied on alone. The history family is one this connector already reaches successfully for order lookups. The SDK types that payload `any`, so the first element seen per process is logged verbatim (through `safeStringify`) — the mapping is meant to be checked against what Kraken actually sends, because reading an unverified field through `as any` is exactly what caused the defect this endpoint exists to measure.
