...
describe('partial-fill', () => {
  ...
  expect(
    'mapOrderStatus(partiallyFilled)',
    ex.mapOrderStatus('partiallyFilled'),
    'PARTIALLY_FILLED',
  )
  expect('mapOrderStatus(untouched)', ex.mapOrderStatus('untouched'), 'NEW')
  expect(
    'mapOrderStatus(ENTERED_BOOK)',
    ex.mapOrderStatus('ENTERED_BOOK'),
    'NEW',
  )
  expect(
    'mapOrderStatus(FULLY_EXECUTED)',
    ex.mapOrderStatus('FULLY_EXECUTED'),
    'FILLED',
  )
  ...
  expect(
    'mapOrderStatus(PARTIALLY_FILLED)',
    ex.mapOrderStatus('PARTIALLY_FILLED'),
    'PARTIALLY_FILLED',
  )
  ...
  expect(
    'primary ENTERED_BOOK + partial fill',
    primaryPartial.status,
    'PARTIALLY_FILLED',
  )
  ...
  expect(
    'derive fully filled',
    ex.futures_deriveOrderStatus('ENTERED_BOOK', 1, 1),
    'FILLED',
  )
  expect(
    'derive untouched no fill',
    ex.futures_deriveOrderStatus('untouched', 0, 1),
    'NEW',
  )
  expect(
    'derive cancelled beats fill',
    ex.futures_deriveOrderStatus('CANCELLED', 0.4, 1),
    'CANCELED',
  )
})
