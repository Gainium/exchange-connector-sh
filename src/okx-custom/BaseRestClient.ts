...
import { serializeParams } from 'okx-api/lib/util/requestUtils'
import { isRawAPIResponse } from 'okx-api/lib/util/typeGuards'
...
export default abstract class BaseRestClient {
  ...
  public postPrivate(endpoint: string, params?: any) {
    return this._call(
      'POST',
      endpoint,
      Array.isArray(params) ? params.map((p) => ({ ...p })) : { ...params },
      false,
    )
  }
   ...
   * Sign request
   */
  private async signRequest<T extends Record<string, any>>(
    isPublicApi: boolean,
    tsISO: string,
    method: Method,
    endpoint: string,
