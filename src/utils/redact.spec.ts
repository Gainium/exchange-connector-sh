 ...
 *
 * The shapes below are the real ones the SDKs throw (see `redact.ts` for the
 * `@siebly/kraken-api` call chain and the bybit-custom / bitget-custom
 * `parseException`s), reconstructed with SYNTHETIC key material. Never paste
 * real credentials in here — this file is committed to a public repo, and the
 * whole point of the module under test is that key material does not get
 * written down.
 *
...
// 1) The Kraken shape: credentials ride inside requestParams.options.headers,
//    right beside the `APIKey` field the SDK does redact.
const krakenError = {
  code: 520,
...
// 3) Suffix matching must cover every vendor's header prefix without a list.
for (const header of [
  'OK-ACCESS-KEY',
  'OK-ACCESS-SIGN',
...
// 4) Ordinary diagnostic fields must NOT be redacted — a log that hides the
...
// 5) The logging path must never throw or hang on a hostile object.
